...
- Foundation environment textures are now modified CC0 Kenney derivatives generated by `scripts/generate-foundation-metal-textures.mjs`, with `sourceSha256` preserving the original source file and `sha256`/`optimizedSha256` tracking the committed derivative.
- The active foundation environment texture payload is now 764229B across floor, wall, and roof PNGs, still under the 1 MB MVP environment payload gate.
- Latest `npm run validate:assets` passed after recording the generated metal texture bytes and hashes.
- Added a Meshy-generated main-character source pack as staged player-character provenance, with `runtimeIntegrated: false` in the ledger and no gameplay load-group wiring.
- Player-character source payload is tracked separately from initial level payload because the unoptimized animation GLBs are not shippable runtime assets yet.
...
- Keep `scripts/generate-foundation-metal-textures.mjs` as the reproducible source for the modified metal PNGs.
...
- Earlier foundation texture records used matching `sourceSha256`, `optimizedSha256`, and `sha256` for unmodified source copies; the current derivative textures intentionally keep source and committed hashes separate.
- `validate-assets` now allows texture records marked `modified: true` to keep source and committed hashes separate.
- New enemy model records use WebP previews only for source snapshots; runtime does not load those thumbnails.
- Future generated texture edits must update the source ledger and rerun `npm run validate:assets`.
- Future player-character runtime manifests should consume an optimized/merged GLB, not the current per-animation Meshy source GLBs.
